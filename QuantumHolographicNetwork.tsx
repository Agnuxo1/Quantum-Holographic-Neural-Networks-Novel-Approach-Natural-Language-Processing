...
export default function Component() {
  ...
  }, [memory, saveMemory, isHuggingFaceConnected, huggingFaceApiKey])

  const getRelevantContext = (input: string) => {
    const words = input.toLowerCase().match(/\b\w+\b/g) || [];
    ...
    const context = uniqueDocuments.slice(0, 5).map(docId => {
      // Here you would retrieve the actual document content based on the docId
      // For this example, we'll just return the docId as a placeholder
      return `Document ${docId}: This is a placeholder for the document content.`;
    }).join('\n\n');
